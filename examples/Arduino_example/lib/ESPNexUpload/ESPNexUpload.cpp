...
bool ESPNexUpload::upload(const uint8_t *file_buf, size_t buf_size) {
    ...
    uint8_t timeout = 0;
    std::string response = "";
	static uint8_t sent_bulk_counter = 0;
    ...
    for(uint32_t i = 0; i < buf_size; i++){
		...
		if(_sent_packets == 4096 || _sent_packets_total >= _undownloadByte){
			...
			uartWrite(c);    // write byte to nextion over serial
...
uint32_t ESPNexUpload::uartAvailable() {
    ...
    if (_upload_uart_has_peek) available++;
    uart_mutex_unlock();
    return available;
}
